      ...
      >
        <box flexDirection="row" flexGrow={1} minHeight={0}>
          ...
          <Show when={sidebarVisible()}>
            <Switch>
              <Match when={wide()}>
               {/* <text>*******    侧边栏（响应式: 宽屏 inline / 窄屏 overlay） *******</text> */}
  ...
  return (
    <>
      <For each={props.parts}>
        {(part, index) => {
          // component 有 ： TextPart 组件  ToolPart 组件  ReasoningPart 组件
          const component = createMemo(() => PART_MAPPING[part.type as keyof typeof PART_MAPPING])
          global.myLog(part, 'part-------------')
          return (
            <Show when={component()}>
              <text>*******  助手消息-a *******</text>
              <Dynamic
...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  // Solid.js 的组件是 “蓝图（Blueprint）” 模式：
  // 组件函数只在最开始执行一次，用来构建响应式节点的关系图。之后数据变化，
