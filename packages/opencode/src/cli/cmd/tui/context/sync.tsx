...
/***
 * sync.tsx 是 TUI 的全局数据中心 + 事件驱动的实时同步引擎，核心职责：
    1. 全局状态存储 — 通过 SolidJS createStore 统一管理整个 TUI 的所有数据（session、message、provider、agent、config、permission、todo、lsp、mcp、vcs 等）
    2. 事件驱动更新 — 订阅后端 SSE 事件（permission.*, session.*, message.*, message.part.*, todo.updated 等），实时增删改 store 中的数据
    3. 启动引导 (bootstrap) — onMount 时批量拉取初始化数据，支持 OPENCODE_FAST_BOOT 跳过等待
    4. 按需会话同步 — session.sync(id) 全量同步某个 session 的消息/parts/todo/diff，避免重复同步
 * 
 * 
 * ***/
