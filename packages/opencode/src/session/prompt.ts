...
// 这个文件把 Effect 几乎用满了，核心目的就一个：*把"副作用管理"从"隐式魔法"变成"显式类型"*。
...
/**
 * Layer.effect 作用是声明依赖 + Service.of(...）装盘！
 * 
 * Service.of(...）给 Service 装盘，其他模块使用这个Service的时候，就可以直接调用:
 *   Service.cancel 
 *   Service.prompt 
 *   Service.loop 
 *   ...
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    /**
     * Effect 里的 Service 不是"监听端口处理请求"的那种传统服务。它更准确的名字应该是**"可注入的能力模块"**。
     * 
     * yield* 都是这个服务依赖的模块，Interface 定义的 Service 接口中都有使用到！
     * 
     * 
     * **/
