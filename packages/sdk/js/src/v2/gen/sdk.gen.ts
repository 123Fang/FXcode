...
// dk.gen.ts 是给前端调用后端 API 的"电话本"，
// 里面列好了所有可用的接口（session.prompt、session.list 等），
// 每个接口都是对 HTTP 请求的封装。自动生成，不要手动改。
...
export class Session2 extends HeyApiClient {
  ...
  // sdk.client.session.prompt(...) 就是调用这个方法！！！
  // 底层通过 postmessage 传给 worker线程的onmessage,
  // worker 的 onmessage 监听函数收到后主线程的数据，调用后端自己的rpc，
  // 后端rpc定义有多个方法 (fetch , sever...),
  // rpc中调用 const response = await  Server.Default().app.fetch(request) 发送给路由服务,
  // 会根据path执行对应的handler（不走网络，本地函数调用）
  public prompt<ThrowOnError extends boolean = false>(
    ...
    // client.post(...)   client.get(...)  ---- 是在这里消费的 ！！！
